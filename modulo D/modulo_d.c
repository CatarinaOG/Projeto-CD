...
int initDecompressRLE(char *path_rle, char modo, int tam_bloco_input){
    ...
    char *path_original;
    findPath(path_rle,&path_original,NULL,4,0); /*Remove .rle*/
    FILE *lista_fps[3];
    ...
    if(modo == 'L'){
        char *path_freq; 
        findPath(path_rle,&path_freq,".freq",0,5);
        FILE *fp_freq = fopen(path_freq,"r"); CheckFile(fp_freq, lista_fps, 2, path_freq); lista_fps[2] = fp_freq;
        bufferSizesRLE(fp_freq,&buffer_sizes_rle); /* Constroi array com os tamanhos dos diferentes blocos */
        free(path_freq); 
        fclose(fp_freq);
    }
    else{
        CheckPointer(buffer_sizes_rle  = (int *) malloc(sizeof(int)));
        *buffer_sizes_rle = tam_bloco_input; /* Tamanho definido pelo utilizador */
    }
    
    r = decompressRLE(fp_rle, fp_original, buffer_sizes_rle, modo);

    path_final = path_original; /* Guardado o path do ficheiro resultante da descompressao na variavel global, para posteriormente ser escrito no terminal*/
...
int decompressSF_RLE(char modo, char *path_shaf){
    int r; char *path_new, *path_cod, check_RLE;

    findPath(path_shaf,&path_new,NULL,5,0);
    findPath(path_shaf,&path_cod,".cod",5,4);
    FILE *lista_fps[4];
    ...
    fscanf(fp_cod,"@%c@%d@", &check_RLE,&nr_blocos); /* Fetch do char que indica se houve compressao RLE e do nº de blocos */

    if(modo == 'S' || check_RLE == 'N') { /* Caso em que não houve compressao RLE, ou em caso de apenas se pretender fazer a descompressao SF */
        r = decompressSF(fp_shaf,fp_cod,fp_new,NULL);
        path_final = path_new; /* Guardado o path do ficheiro resultante da descompressao na variavel global, para posteriormente ser escrito no terminal*/
    }
    else{
        char *path_original; findPath(path_new,&path_original,NULL,4,0);
